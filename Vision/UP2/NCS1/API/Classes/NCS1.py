...
class NCS1():
	...
	def __init__(self):
		...
		self.Predictor = dlib.shape_predictor(os.path.dirname(
			os.path.abspath(__file__)) + '/../' + self.Helpers.confs["Classifier"]["Dlib"])
	...
	def load(self):
		...
		with open(os.path.dirname(os.path.abspath(__file__)) + '/../' + self.Helpers.confs["Classifier"]["Graph"], mode='rb') as f:
			graphFile = f.read()
	...
	def preprocess(self):
		...
		for filename in os.listdir(os.path.dirname(os.path.abspath(__file__)) + '/../' + self.Helpers.confs["Classifier"]["Known"]):
			# Checks file type
			if filename.lower().endswith(tuple(self.Helpers.confs["Classifier"]["Allowed"])):
				fpath = os.path.join(
					os.path.dirname(os.path.abspath(__file__)) + '/../' + self.Helpers.confs["Classifier"]["Known"], filename)
	...
	def prepareImgRemote(self, frame):
		""" Reads & processes frame from the local GeniSysAI. """
	...
	def match(self, frame, coords):
		...
		for enc in self.encoded:
			...
			if recognize[0] == True:
				person = int(enc[0])
				distance = recognize[1]
				msg = "GeniSysAI identified User #" +  str(person)
				break

		if(person == 0):
			msg = "GeniSysAI identified an intruder"
