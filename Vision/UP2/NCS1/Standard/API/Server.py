...
# Project:       NCS1 API Security System
#
...
class Server():
	...
	def life(self):
		...
		self.Helpers.logger.info(
			"GeniSysAI Facial API Life (TEMPERATURE): " + str(tmp) + "\u00b0")
		self.Helpers.logger.info(
			"GeniSysAI Facial API Life (CPU): " + str(cpu) + "%")
		self.Helpers.logger.info(
			"GeniSysAI Facial API Life (Memory): " + str(mem) + "%")
		self.Helpers.logger.info(
			"GeniSysAI Facial API Life (HDD): " + str(hdd) + "%")
	...
	def threading(self):
		""" Starts the GeniSysAI Facial API software threads. """
...
def Inference():
	...
	if len(coords):
		...
		for (i, face) in enumerate(coords):
			...
			if known is not 0:
				idd += 1
				msg = "GeniSysAI identified User #" + str(known)
			else:
				intruders += 1
